...
def LandmarkError(imageServer, faceAlignment, extraLandmarks, OrgsInd, resultsDir, normalization='centers', showResults=False, verbose=False):
    errors = []
    ...
    for i in range(nImgs):
        ...
        newShapeDeformed = faceAlignment.processImgWithmoms(img, initLandmarks, extraLandmarks, OrgsInd)
        ...
        if showResults:
            ...
            # plt.show()
            # plt.clf()
            fig.savefig(resultsDir + 'Img' + str(i) + '.png')

            fig1 = plt.figure()
            plt.imshow(img[0], cmap=plt.cm.gray)
            plt.plot(newShapeDeformed[:, 0], newShapeDeformed[:, 1], 'o')
            # plt.show()
            # plt.clf()
            fig1.savefig(resultsDir + 'Img' + str(i) + '_deformed.png')
