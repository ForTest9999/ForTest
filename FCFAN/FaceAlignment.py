...
class FaceAlignment(object):
    ...
    def initializeNetwork(self):
        ...
        self.network_moms = self.layers['s1_output']

        self.prediction_moms = lasagne.layers.get_output(self.network_moms, deterministic=True)
        self.generate_network_moms = theano.function([self.data], [self.prediction_moms])
    ...
    def processImgWithmoms(self, img, inputLandmarks, extraLandmarks, OrgsInd):
        ...
        output_moms = self.generate_network_moms([inputImg])[0][0]
        moms = output_moms.reshape((-1, 2))
